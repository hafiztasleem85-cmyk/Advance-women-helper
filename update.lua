...
local showHealthTracker, showMainMenu, openManualDateDialog, showCountryList, showHelplines, showNumberDetail, showHelplinesDialog
...
pcall(function() offlineData = JSONObject(defaultIndiaData) end)
elseif country == "Pakistan" then
pcall(function() offlineData = JSONObject(defaultPakistanData) end)
elseif country == "Bangladesh" then
pcall(function() offlineData = JSONObject(defaultBangladeshData) end)
elseif country == "Nepal" then
pcall(function() offlineData = JSONObject(defaultNepalData) end)
elseif country == "Sri Lanka" then
pcall(function() offlineData = JSONObject(defaultSriLankaData) end)
elseif country == "China" then
pcall(function() offlineData = JSONObject(defaultChinaData) end)
elseif country == "Indonesia" then
pcall(function() offlineData = JSONObject(defaultIndonesiaData) end)
elseif country == "Malaysia" then
pcall(function() offlineData = JSONObject(defaultMalaysiaData) end)
elseif country == "Nigeria" then
pcall(function() offlineData = JSONObject(defaultNigeriaData) end)
elseif country == "South Africa" then
pcall(function() offlineData = JSONObject(defaultSouthAfricaData) end)
elseif country == "UK" then
pcall(function() offlineData = JSONObject(defaultUKData) end)
elseif country == "Ukraine" then
pcall(function() offlineData = JSONObject(defaultUkraineData) end)
elseif country == "USA" then
pcall(function() offlineData = JSONObject(defaultUSAData) end)
end
end
if not offlineData then
...
local menuOptions = {"Women Helplines", "Health & Period Tracker"}
local bMenu = AlertDialog.Builder(service)
bMenu.setTitle("Advance Women Helper")
bMenu.setItems(menuOptions, DialogInterface.OnClickListener{
onClick = function(d, w)
d.dismiss()
if w == 0 then showCountryList()
elseif w == 1 then showHealthTracker() end
end
})
